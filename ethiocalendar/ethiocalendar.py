...
# also assumes the current Ethiopian calendar indefinitely extended in
# both directions. 
...
def _days_in_month(year, month):
    ...
    assert 1 <= month <= 13, month
    if month == 13:
        return 6 if is_puagume6(year) else 5
    return 30

def _days_before_month(year, month):
    "year, month -> number of days in year preceding first day of month."
    assert 1 <= month <= 13, 'month must be in 1..13'
    return 30 * (month - 1)

def _ymd2ord(year, month, day):
    "year, month, day -> ordinal, considering 01-01-0001 as day 1."
    assert 1 <= month <= 13, 'month must be in 1..13'
...
_MONTHNAMES = [None, "Meskerem", "Tikimit", "Hidar", "Tahsas", "Tir", "Yekatit",
                     "Megabit", "Miyaziya", "Ginbot", "Sene", "Hamle", "Nehase", "Puagme"]
_DAYNAMES = [None, "Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]


def _build_struct_time(y, m, d, hh, mm, ss, dstflag):
    wday = (_ymd2ord(y, m, d) + 6 + WEEKDAYADVANCE) % 7
...
def _check_date_fields(year, month, day):
    ...
    if not 1 <= month <= 13:
        raise ValueError('month must be in 1..13', month)
    dim = _days_in_month(year, month)
    if not 1 <= day <= dim:
...
class date:
    ...
    def fromordinal(cls, n):
        """Construct a date from a Ethiopian ordinal calendar.

        Meskerem 1 of year 1 is day 1.  Only the year, month and day are
    ...
    def fromisocalendar(cls, year, week, day):
        ...
        # Year is bounded this way because 9999-13-6 is (9999, 52, 5)
        if not MINYEAR <= year <= MAXYEAR:
        ...
        if not 0 < week < 53:
            ...
            if week == 53:
                ...
                first_weekday = (_ymd2ord(year, 1, 1) + WEEKDAYADVANCE) % 7
    ...
    def ctime(self):
        ...
        weekday = (self.toordinal() + WEEKDAYADVANCE) % 7 or 7
    ...
    def togregorian(self):
        return _gregoriancalendar.date.fromordinal(self.toordinal() + ETHIOORIGINALDAYDELAY)
    ...
    def toordinal(self):
        """Return proleptic Ethiopian ordinal for the year, month and day.

        Meskerem 1 of year 1 is day 1.  Only the year, month and day values
    ...
    def weekday(self):
        ...
        return (self.toordinal() + 6 + WEEKDAYADVANCE) % 7

    # Day-of-the-week and week-of-the-year, according to ISO

    def isoweekday(self):
        "Return day of the week, where Monday == 1 ... Sunday == 7."
        return (self.toordinal() + WEEKDAYADVANCE) % 7 or 7
...
datetime.max = datetime(9999, 13, 6, 23, 59, 59, 999999)
...
def _isoweek1monday(year):
    ...
    firstweekday = (firstday + 6 + WEEKDAYADVANCE) % 7  # See weekday() above
...
try:
    from _ethiocalendar import *
except ImportError:
    pass
else:
    # Clean up unused names
    del (_DAYNAMES, _DAYS_BEFORE_MONTH, _DAYS_IN_MONTH,
        _EPOCH, _MAXORDINAL, _MONTHNAMES, _build_struct_time,
         _check_date_fields, _check_int_field, _check_time_fields,
         _check_tzinfo_arg, _check_tzname, _check_utc_offset, _cmp, _cmperror,
         _date_class, _days_before_month, _days_before_year, _days_in_month,
         _format_time, _format_offset, is_puagume6, _isoweek1monday, _math,
    ...
    from _ethiocalendar import __doc__
